...
import type { PostDTO, PostsSummary } from '../api-types.js';
      ...
      {summary && (
        ...
        >
          ...
          {summary.totalComments > 0 && (
            <> &middot; {summary.totalComments.toLocaleString()} comments</>
          )}
        </div>
      )}
              ...
              <div style={{ fontSize: '13px', color: '#64748b', marginBottom: '8px' }}>
                ...
                {post.url && (
                  <>
                    {' '}
                    &middot;{' '}
                    <a href={post.url} target="_blank" rel="noopener" style={{ color: '#3b82f6' }}>
                      View
                    </a>
                  </>
                )}
              ...
              Object.keys(post.metrics).length > 0 && (
                <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '8px' }}>
                  {post.metrics.views != null && (
                    <span>Views: {post.metrics.views.toLocaleString()} </span>
                  )}
                  {post.metrics.likes != null && <span>&middot; Likes: {post.metrics.likes} </span>}
                  {post.metrics.shares != null && (
                    <span>&middot; Shares: {post.metrics.shares} </span>
                  )}
                  {post.metrics.comments != null && (
                    <span>&middot; Comments: {post.metrics.comments} </span>
                  )}
