...
import type { VerticalDTO } from '../api-types.js';
...
export function VerticalManagement() {
  const [verticals, setVerticals] = useState<VerticalDTO[]>([]);
        ...
        <div key={v.id} style={cardStyle}>
          ...
          {v.accounts?.length > 0 && (
            <div style={{ fontSize: '13px', color: '#94a3b8', marginTop: '8px' }}>
              Accounts: {v.accounts.map(a => `${a.name} (${a.platform})`).join(', ')}
            </div>
          )}

          <div style={sectionStyle}>
            <div
              style={{ fontSize: '13px', fontWeight: 600, color: '#94a3b8', marginBottom: '6px' }}
            >
              Data Sources
            </div>
            {v.dataSources?.map(ds => (
              <div key={ds.id} style={rowStyle}>
                ...
                <span style={{ color: '#64748b' }}>
                  ...
                  {ds.lastPolledAt && (
                    <> &middot; Last: {new Date(ds.lastPolledAt).toLocaleTimeString()}</>
                  )}
                </span>
              </div>
            ))}
              ...
              <div key={rule.id} style={rowStyle}>
                <div>
                  ...
                  <span style={{ color: '#64748b', fontSize: '12px', marginLeft: '8px' }}>
                    ...
                    {rule.lastFiredAt && (
                      <> &middot; Last: {new Date(rule.lastFiredAt).toLocaleTimeString()}</>
                    )}
                  </span>
                </div>
          ...
          <div style={sectionStyle}>
            ...
            {v.contentTemplates?.map(t => (
              <div key={t.id} style={rowStyle}>
                <span>{t.name}</span>
                <span style={{ color: '#64748b' }}>
                  {t.contentLayer} &middot; {t.category}
                </span>
              </div>
            ))}
          </div>
